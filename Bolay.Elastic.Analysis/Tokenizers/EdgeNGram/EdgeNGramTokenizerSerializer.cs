...
{
    internal class EdgeNGramTokenizerSerializer : JsonConverter
    {
        ...
        public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            ...
            EdgeNGramTokenizer token = new EdgeNGramTokenizer(tokenDict.First().Key);
            TokenizerBase.Deserialize(token, fieldDict);
            token.MinimumSize = fieldDict.GetInt64(_MINIMUM_GRAM, EdgeNGramTokenizer._MINIMUM_SIZE_DEFAULT);
            token.MaximumSize = fieldDict.GetInt64(_MAXIMUM_GRAM, EdgeNGramTokenizer._MAXIMUM_SIZE_DEFAULT);
        ...
        {
            if (!(value is EdgeNGramTokenizer))
                throw new SerializeTypeException<EdgeNGramTokenizer>();

            EdgeNGramTokenizer token = value as EdgeNGramTokenizer;

            Dictionary<string, object> fieldDict = new Dictionary<string, object>();
            TokenizerBase.Serialize(token, fieldDict);
            fieldDict.AddObject(_MINIMUM_GRAM, token.MinimumSize, EdgeNGramTokenizer._MINIMUM_SIZE_DEFAULT);
            fieldDict.AddObject(_MAXIMUM_GRAM, token.MaximumSize, EdgeNGramTokenizer._MAXIMUM_SIZE_DEFAULT);
